...
def teacher_forward(model, src_sequence, vocab,
                    max_len=148,
                    device="cpu",
                    criterion = None,
                    optimizer = None,
    ...
    tgt = [vocab.sos_idx] + [vocab.pad_idx] * (max_len-1)
    tgt = torch.tensor([tgt], device=device)
    tgt = tgt.repeat(src.size(0), 1)

    #logits = torch.zeros(src.size(0), max_len, len(vocab), device=device)
    ...
    for i in range(1,max_len+1):
        ...
        encoder_output = model.encoder(src[active_indxs], src_mask[active_indxs])
        ...
        loss = criterion(decoder_output.permute(0, 2, 1), src_sequence[active_indxs,0:i])
        loss.backward()
        clip_grad_norm_(model.parameters(), 1)
        optimizer.step()

        total_loss += loss.item()
        total_loss /= src[active_indxs].size(0)


        # Get next token prediction
        next_token = decoder_output.argmax(dim=-1)
        # store logits
        #logits[active_indxs,i-1,:] = decoder_output[:,i-1,:]
    ...
    #target = torch.zeros(src.size(0), max_len, dtype=torch.long, device=device)
...
# def teacher_forward(model, src_sequence, vocab, max_len=148, device="cpu"):
#     """
#     Generates predictions for a batch of source sequences using the given model.
#     Args:
#         model (torch.nn.Module): The model used for generating predictions.
...
#     #logits = torch.zeros(src.size(0), max_len, len(vocab), device=device, requires_grad=True)
...
#         # decode
#         decoder_output = model.decoder(tgt,
...
def predict_batch(model, src_sequence, vocab, max_len=148, device="cpu"):
    """
    Generates predictions for a batch of source sequences using the given model.
    Args:
        model (torch.nn.Module): The model used for generating predictions.
